...
- **RESOLVED, and the numbers in this table are the correct ones.** The investigation above
  correctly localised the fault to the calibration side; the calibration table was generated by
  an ad-hoc script that called `apply_quality_control` directly and **skipped
  `preprocess_sweep`**, which the shipping ingest path (`src/server.py::_ingest_to_buffer`) runs
  before it. That stage's `_remove_weak_speckle` blanks isolated >=20 dBZ components of <=3 gates
  (peak < 35 dBZ) to NaN, along with their co-registered fields.

  This explains every symptom precisely. Storm-layer figures are untouched either way, because a
  <=3-gate component could never clear object detection's 4 km2 area floor -- hence the
  bit-for-bit storm match, including the decimal. The precipitation layer has no area floor, so
  those blanked gates trim contour edges and punch holes in exactly the low bands where isolated
  speckle lives -- hence a mismatch confined to the two largest, most fragment-dense Features.

  Reproduced both ways in one process against all three reference volumes:

  | pipeline | storm total | precip total | precip max | precip p95 |
  |---|---|---|---|---|
  | with `preprocess_sweep` (shipping path) | 24,743 | **220,646** | **53,410** | **48,138** |
  | without (how the calibration was made) | 24,743 | 227,887 | 56,875 | 49,613 |

  The same omission is the sole cause of the stale "7,876 pieces / 131,472 vertices" figure that
  `MIN_PRECIP_FRAGMENT_AREA_KM2`'s comment carried (Finding 4) and of the brief's 16,355 dropped
  fragments against this measurement's 14,941. One root cause, three symptoms, all on the
  calibration side.

  The table above stands as measured. A number for this layer is only meaningful if it comes from
  the pipeline that actually serves the user, and this one does.
...
- ~~The precipitation-band vertex-count calibration cross-check did not fully reconcile.~~
  **Resolved after this section was first written** -- the calibration table had been generated by
  a script that skipped `preprocess_sweep`, a stage the shipping ingest path runs. The numbers in
  the "Vertex-count distribution" section above are the correct ones; see the resolution recorded
  there. Kept visible rather than deleted, because the investigation that localised it -- storm
  layer bit-for-bit identical, mismatch confined to the two most fragment-dense low bands -- is
  what made the cause findable, and is worth a future reader's attention if a similar gap appears.
