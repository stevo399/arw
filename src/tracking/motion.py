...
# Superseded threshold of the centre-track motion below, removed with it.
_CENTRE_NEARLY_STATIONARY_KMH = 2.0
...
def compute_motion(positions: list[tuple[datetime, float, float]]) -> MotionVector:
    ...
    if confidence.label == "low" and speed_kmh >= _CENTRE_NEARLY_STATIONARY_KMH:
        return MotionVector(
            speed_kmh=speed_kmh,
    ...
    if speed_kmh < _CENTRE_NEARLY_STATIONARY_KMH:
        return MotionVector(
            speed_kmh=speed_kmh,
            speed_mph=round(speed_kmh / KM_PER_MILE),
            heading_deg=None,
